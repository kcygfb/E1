...
| `CardEnemyAI.cs` | 根据敌方牌组与策略驱动敌方抽牌、出牌、弃牌和回合结束。 |
...
玩家结束回合时，未使用手牌全部进入弃牌堆，然后进入敌方回合。`CardEnemyAI` 根据敌方牌组与策略完成出牌，并调用 `CompleteEnemyTurn` 开始下一回合。
...
当前 `CardDataV2` 实际使用的效果均有解析和规则实现；Schema、解析器与结算器保持一一对应。
