...
│  ├─ StreamingAssets/CardData/  JSON 卡牌库及 Schema
...
日常开发主要关注 `Assets/Script`、`Assets/StreamingAssets/CardData`、`Assets/Data/Combat`、`Assets/Prefabs`、`Assets/Scenes` 和 `Assets/Tests`。`Library`、`Temp`、`obj` 及自动生成的 `.csproj` 不属于业务源码。

## 3. 总体架构与依赖方向

```mermaid
flowchart LR
    JSON["StreamingAssets/CardData\nJSON 卡牌库"] --> Repo["CardJsonRepository\n解析与校验"]
...
| `CardEffectSpec.cs` | 表示一条卡牌效果；`UpgradeableNumber` 保存基础值和强化值。 |
...
| `CombatantState.cs` | 管理角色生命、韧性、行动点、眩晕、减伤、攻击无效和跳过敌方回合等状态。 |
...
| `BattleCardBridge.cs` | 连接规则层手牌与 `CardDealAnimator`，处理初始发牌、出牌、回收和新回合发牌。 |
...
正常流程是先在此处加载 43 张卡牌，选择规定数量的牌，然后进入 `Card`。
...
牌库位于 `Assets/StreamingAssets/CardData`：
...
| 斧 | `axes.json` | 6 |
| 刀剑 | `blades.json` | 7 |
| 软兵器 | `flexible-weapons.json` | 6 |
| 暗器 | `hidden-weapons.json` | 6 |
| 防御 | `defense.json` | 4 |
| 魔法 | `magic.json` | 6 |
| 合计 |  | 43 |
...
- `CardView.SyncCardInteraction` 使用反射同步 `CardInteraction` 的内部字段，后续可改为显式公开接口以降低耦合。
...
| 文件 | 当前覆盖 |
| --- | --- |
| `CardJsonRepositoryTests.cs` | 真实 JSON 牌库可加载 43 张卡牌，分类、强化值和固定值字段正确。 |
...
6. 进入 Play Mode，确认 Console 显示加载 43 张或新的预期数量。
